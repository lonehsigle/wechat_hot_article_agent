...
export async function POST(request: NextRequest) {
  ...
  const { action, articles, styleName, styleId } = body;

  if (action === 'analyze') {
    if (!articles || !Array.isArray(articles) || articles.length === 0) {
      return NextResponse.json({ error: '请提供至少一篇文章进行分析' }, { status: 400 });
    }
...
只输出JSON，不要其他内容。`;
  ...
  if (action === 'save') {
    const { name, analysis } = body;
    if (!name || !analysis) {
      return NextResponse.json({ error: '缺少风格名称或分析结果' }, { status: 400 });
    }
  ...
  if (action === 'save-layout') {
    ...
    if (!name) {
      return NextResponse.json({ error: '缺少排版风格名称' }, { status: 400 });
    }
    ...
    const [saved] = await database.insert(layoutStyles).values({
      ...
      headerStyle: config?.headerStyle || 'bold',
      paragraphSpacing: config?.paragraphSpacing || 'medium',
      listStyle: config?.listStyle || 'number',
      highlightStyle: config?.highlightStyle || 'emoji',
      emojiUsage: config?.emojiUsage || 'moderate',
      quoteStyle: config?.quoteStyle || 'block',
      imagePosition: config?.imagePosition || 'center',
      calloutStyle: config?.calloutStyle || 'box',
    }).returning();
  ...
  if (action === 'delete-layout') {
    const { styleId: layoutStyleId } = body;
    if (!layoutStyleId) {
      return NextResponse.json({ error: '缺少排版风格ID' }, { status: 400 });
    }
